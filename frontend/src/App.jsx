    ...
    <main className="relative min-h-screen overflow-hidden bg-slate-950 px-4 py-6 text-slate-100 sm:px-6 md:py-10">
      ...
      <header className="relative mx-auto mb-8 flex w-full max-w-4xl items-center justify-between rounded-2xl border border-white/10 bg-black/20 px-4 py-3 backdrop-blur-xl">
        <div className="flex items-center gap-2">
          <h1 className="text-sm font-semibold tracking-[0.12em] uppercase mx-auto w-fit rounded-full border text-x border-white/20 bg-white/10 px-2 text-cyan-200">Smart Quiz</h1>
        </div>
        <nav className="flex items-center gap-2">
          <Link className="rounded-lg border border-white/20 px-1 py-1.5 text-sm text-slate-100 hover:bg-white/10" to="*">Главная</Link>
          <Link className="rounded-lg border border-white/20 px-1 py-1.5 text-sm text-slate-100 hover:bg-white/10" to="/analytics">Для разработчиков</Link>
          {isAdmin ? (
            <button
              type="button"
              onClick={() => setAdminToken('')}
              className="rounded-lg border border-white/20 px-1 py-1.5 text-sm text-slate-100 hover:bg-white/10"
            >
              Выйти
            </button>
          ) : null}
